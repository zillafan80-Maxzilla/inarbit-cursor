...
-- 机会配置历史
CREATE TABLE opportunity_config_history (
 id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
 user_id UUID REFERENCES users(id) ON DELETE CASCADE,
 strategy_type strategy_type NOT NULL,
...
CREATE INDEX idx_opportunity_config_history_user ON opportunity_config_history(user_id);
CREATE INDEX idx_opportunity_config_history_type ON opportunity_config_history(strategy_type);

-- 机会配置模板
CREATE TABLE opportunity_config_templates (
 id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
 strategy_type strategy_type NOT NULL,
 name VARCHAR(100) NOT NULL,
 description TEXT,
...
CREATE INDEX idx_opportunity_config_templates_type ON opportunity_config_templates(strategy_type);
